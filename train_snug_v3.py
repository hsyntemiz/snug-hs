...
from losses.physics import gravitational_energy,stretching_energy,bending_energy,collision_penalty,inertial_term_sequence
import smpl
...
class SnugModel(tf.keras.Model):
    ...
    def _build(self):
        ...
        self.gru_layer1 = tf.keras.layers.GRU(256, return_sequences=True, activation='tanh')
        self.gru_layer2 = tf.keras.layers.GRU(256, return_sequences=True, activation='tanh')
        self.gru_layer3 = tf.keras.layers.GRU(256, return_sequences=True, activation='tanh')
        self.gru_layer4 = tf.keras.layers.GRU(256, return_sequences=True, activation='tanh')
...
def train():
    ...
    batch_size = 4  # 16
    tr_data = Dataset.Data(batch_size=batch_size)
    ...
    for epoch in range(num_epochs):
        ...
        for poses, trans, trans_vel in tr_data._iterator:
            ...
            # shape = tf.zeros([bs_size, window, 10], dtype=tf.float32)
            ...
            with tf.GradientTape() as tape:
                ...
                # on the GradientTape.
                translation = tf.reshape(trans, [-1, 3])
                #logits = snug(poses,trans_vel,betas, g_joint_transforms,translation,shape_blends, training=True)  # Logits for this minibatch
                pred1, pred2, pred3, v_body0, body_tensor_dict = snug(
                    shape, poses, trans, trans_vel,
                ...
                energy_stretch = stretching_energy(v=logits,cloth=snug.cloth)  #*0.01#nan donderiyor
                ...
                loss_value = 1 * energy_bend + 1 * g_energy + 1 * inertia + 0.01*energy_stretch + penalty * 1
    ...
    for frame in range(len(poses)):
        ...
        v_garment, v_body0, body_tensor_dict = snug._run(
...
#                 #logits = snug(poses,trans_vel,betas, g_joint_transforms,translation,shape_blends, training=True)  # Logits for this minibatch
#                 pred1, pred2, pred3, v_body0, body_tensor_dict = snug(
#                     shape, poses, trans, trans_vel, hidden_states[0][:bs_size,:], hidden_states[1][:bs_size,:], \
#                      hidden_states[2][:bs_size,:], hidden_states[3][:bs_size,:])
#
#
#                 pred1 = tf.expand_dims(pred1, 1)
#                 pred2 = tf.expand_dims(pred2, 1)
...
#                 penalty = collision_penalty(logits, v_body, tensor_dict["vertex_normals"])
#
#                 loss_value = 1 * energy_bend + 1 * g_energy + 1 * inertia + 0.0*energy_stretch + penalty * 1
#
#
#             # Use the gradient tape to automatically retrieve
#             # the gradients of the trainable variables with respect to the loss.
...
#         )
#
#         # g_joint_transforms = tf.reshape(tensor_dict['joint_transforms'],(bs_size,3,24,4,4))
#         g_joint_transforms = tensor_dict['joint_transforms']
#         shape_blends = tensor_dict['shape_blendshape']
#
#         # translation = tf.reshape(trans, [-1, 3])
#         # v_garment = snug(pose,translation_vel,shape, g_joint_transforms, translation, shape_blends, window=3,training=True)
#         v_garment, v_body0, body_tensor_dict = snug._run(
#             shape, pose, translation, translation_vel, hidden_states[0], hidden_states[1], \
#             hidden_states[2], hidden_states[3])
#
